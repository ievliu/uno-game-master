...
class EventHandlerModule {
    ...
    onConnection(client: Socket) {
        try {
            ...
            SocketService.on<unknown, CreateGameEventResponse>(
                client,
                "CreateGame",
                async () => {
            ...
            SocketService.on<unknown, CreateBoringGameEventResponse>(
                client,
                "CreateBoringGame",
                async () => {
                    let game = await GameService.getExistingPlayerGame(
                    ...
                    if (!game) {
                        ...
                        game = await GameService.setupBoringGame(
                            playerData.id,
                            chat.id
                        );
                    }
            ...
            SocketService.on<JoinGameEventInput, JoinGameEventResponse>(
                client,
                "JoinGame",
                async ({ gameId }) => {
            ...
            SocketService.on<BuyCardEventInput, unknown>(
                client,
                "BuyCard",
                async ({ gameId }) => {
            ...
            SocketService.on<PutCardEventInput, unknown>(
                client,
                "PutCard",
                async ({ gameId, cardIds, selectedColor }) => {
                    await GameService.putCard(
                        playerData.id,
                        cardIds,
                        gameId,
                        selectedColor
            ...
            SocketService.on<SendChatMessageEventInput, unknown>(
                client,
                "SendChatMessage",
            ...
            SocketService.on<ChangePlayerStatusEventInput, unknown>(
                client,
                "ChangePlayerStatus",
                async ({ gameId, playerStatus }) => {
                    await GameService.changePlayerStatus(
                        gameId,
                        playerData.id,
                        playerStatus
                    );
            ...
            SocketService.on<ToggleReadyEventInput, unknown>(
                client,
                "ToggleReady",
                async ({ gameId }) => {
            ...
            SocketService.on<ForceSelfDisconnectEventInput, unknown>(
                client,
                "ForceSelfDisconnect",
                async () => {
                    const purgedGames = await GameService.purgePlayer(
                        playerData.id
                    );
            ...
            SocketService.on<unknown, unknown>(
                client,
                "disconnect",
                async () => {
